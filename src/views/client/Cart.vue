...
// Utilisation de computed avec getter/setter pour lier les inputs directement au store Pinia
const customerName = computed({
  get: () => cartStore.customerName,
  set: (val) => { cartStore.customerName = val }
})

const tableNumber = computed({
  get: () => cartStore.tableNumber,
  set: (val) => { cartStore.tableNumber = val }
})

const specialNotes = computed({
  get: () => cartStore.specialNotes,
  set: (val) => { cartStore.specialNotes = val }
})
...
const handleCheckout = async () => {
  ...
  await orderStore.submitOrder(orderPayload)
  
  if (!orderStore.error) {
    cartStore.clearCart() // Vide le panier ET réinitialise le nom, la table et les notes
    router.push('/confirmation')
  } else {
    errorMessage.value = orderStore.error
  }
}
</script>
  ...
  <div class="cart-page">
    <div class="container">
      ...
      <div v-else class="cart-content">
        <!-- Lien pour ajouter un autre plat -->
        <router-link to="/menu" class="add-more-link">
        ...
        <form @submit.prevent="handleCheckout" class="checkout-form">
          ...
          <div class="cart-summary">
            ...
            <div class="total-amount">{{ (cartStore.totalPrice || cartStore.totalAmount || 0).toLocaleString() }} FCFA</div>
            <button 
              type="submit" 
              class="btn-submit" 

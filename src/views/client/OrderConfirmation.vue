    ...
    <div class="container">
      ...
      <div class="confirmation-card" v-if="lastOrder">
        ...
        <div class="summary-box">
          ...
          <div class="summary-line" v-for="item in lastOrder.items" :key="item.product?.id || item.id">
            <span>{{ item.product?.name || item.name }} x{{ item.quantity }}</span>
            <span>{{ (((item.price || 0) * (item.quantity || 0))).toLocaleString() }} FCFA</span>
          </div>

          <div class="summary-line fee-line" v-if="lastOrder.serviceFee">
            <span>Frais de service</span>
            <span>{{ lastOrder.serviceFee?.toLocaleString() || 0 }} FCFA</span>
          </div>

          <div class="summary-total">
            <span>Total</span>
            <span class="total-price">{{ lastOrder.total?.toLocaleString() || 0 }} FCFA</span>
          </div>
        </div>
